...
- Common Good Extras is its own shopping step/category and should include jars, bottles, brushes, loofahs, wool dryer balls, miswak toothbrushes, and other useful goods rather than only glass.
...
- Product setup should eventually move to one owner intake screen with all relevant fields: product name, department/category, product status, materials, recipe, labor, waste, packaging, label assets, suggested price, actual price, refill format, Pantry Ready format, locations, inventory split, and social media aspects. Product status options include Active, Testing, Future Product, Inactive, and Discontinued; only Active products should appear on customer-facing screens.
...
- If a unique Common Good jar is purchased, excluding basic mason/Ball-style jars, it should eventually be added to the customer's saved jars when customer accounts exist.
...
- Reports should be buried inside existing screens rather than becoming another top-level tab. Sales Ledger includes date range, product, location, payment method, format, variance/discrepancy, and donation filters. Inventory includes category, location, status, reorder, and sellable-extra filters.
